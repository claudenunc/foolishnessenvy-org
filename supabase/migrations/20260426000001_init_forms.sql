...
-- Created: 2026-04-26
-- Target: fresh Supabase project at dlqjwnbyepbenyltyrku.supabase.co
...
create index contact_submissions_sms_opt_in_idx
    on public.contact_submissions (sms_opt_in) where sms_opt_in = true;
...
create unique index newsletter_signups_email_uniq
...
create index newsletter_signups_sms_opt_in_idx
    on public.newsletter_signups (sms_opt_in) where sms_opt_in = true;

-- ============================================================
-- donations_intent
-- ============================================================
create table public.donations_intent (
    id                 uuid           primary key default gen_random_uuid(),
    created_at         timestamptz    not null default now(),
    name               text           not null,
    email              text           not null,
...
create index donations_intent_created_at_idx
    on public.donations_intent (created_at desc);
create index donations_intent_sms_opt_in_idx
    on public.donations_intent (sms_opt_in) where sms_opt_in = true;

-- ============================================================
-- Row Level Security — allow anon inserts only; reads need service role
-- ============================================================
